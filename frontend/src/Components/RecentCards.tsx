    ...
    <div className={`rounded-2xl my-4 p-4 ${drk_theme?"bg-bg_dark_pri text-font_dark_pri":"bg-bg_light_pri text-font_light_pri"}`}  >
        <div className='flex items-center ' >
            ...
            <div className='mx-2 w-5/6'>
                <p className='font-bold hover:text-third_color cursor-pointer w-fit' onClick={()=>handleUserProfile(data?.user?.id)}>{data?.user.email}</p>
                <p className={`${drk_theme?"text-font_dark_sec":"text-font_light_sec"} text-sm font-semibold`}>{CalcTime(dynamicTime)}</p>
                <p onClick={()=>handleGotoRoom(data?.room?.id)} className={`${drk_theme?"text-font_dark_sec":"text-font_light_sec"} text-sm font-semibold cursor-pointer`} >replied in: <span className={`font-bold`}>{data?.room?.name}</span> </p>
            </div>
        </div>
