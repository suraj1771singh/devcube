...
>>>>>>> d108c9dc0c9b3de177615ecafce278dacf3dbe1c
            ...
            <div className='flex justify-between mt-6' >
                ...
                <div className='flex'>
                    {data?.topic?.map((el: topicDataType) => <div key={el.id} className={`flex justify-center items-center px-6 py-2 font-semibold rounded-full mx-2 ${drk_theme ? "bg-bg_dark_pri text-font_dark_pri" : "bg-bg_light_pri text-font_light_pri"}`} >{el.name}</div>)}
                </div>
            </div>
        </div>
    )
