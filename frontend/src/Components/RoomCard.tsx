import { useDispatch, useSelector } from 'react-redux';
import { rootReducertype } from '../Redux/Store';
import { CalcTime } from './time';
import { GrGroup } from 'react-icons/gr'
import { FiMoreHorizontal } from 'react-icons/fi'
import { NavLink, useNavigate } from 'react-router-dom';
import { Dispatch, useEffect, useState } from 'react';
import { joinRoom} from '../Redux/room/room.action';

const RoomCard = ({data}: any) => {
    const dispatch:Dispatch<any> = useDispatch()
    let { drk_theme } = useSelector((val: rootReducertype) => val.theme)
    // const { allTopics } = useSelector((val: rootReducertype) => val.topics)
    const{myData} =  useSelector((val:rootReducertype)=>val?.auth)
    const [joined,setJoined] = useState(false)
    const [owner,setOwner] = useState(false)
    const nav = useNavigate()
    const {join_loading} = useSelector((val:rootReducertype)=>val.rooms)
    useEffect(()=>{
       setJoined(data?.participants.some((el:any)=>el.id===myData?.user_id))
       if(data.host.id===myData?.user_id){
        setOwner(true)
       }
    },[data.host.id, data.participants,data.participants.length, myData])
    let date = data.created;
    let dynamicTime = new Date(date).getTime()
    const handleJoinRoom = (id:string|number)=>{
      let user = {id:myData.user_id,email:myData.username}
       dispatch(joinRoom(id,user))
    }
    const handleGotoRoom = (id:string|number)=>{
        nav(`/room/${id}`)
    }
    return (
        <div className={`my-3 ${drk_theme ?"bg-bg_dark_sec text-font_dark_pri" : "bg-bg_light_sec text-font_light_pri"} rounded-2xl p-6  hover:shadow-xl`} >
            <div className='flex justify-between items-center'>
                <div className='flex items-center'>
                    <div className='border-2 relative border-third_color w-[50px] h-[50px] rounded-full'>
                        <img src="/profile.svg" alt="dp" />
                    </div>
                    <NavLink to={`/profile/${data.host}`} className='mx-2 hover:text-third_color cursor-pointer'>@{data?.host.email} {owner?"(You)":""} </NavLink>
                </div>
                <div className={`flex flex-col items-end h-[100%]`}>
                    <FiMoreHorizontal className='text-xl cursor-pointer hover:text-third_color' />
                    <p className='text-xs font-semibold'>
                        {CalcTime(dynamicTime)}
                    </p>
                </div>
            </div>
            <div onClick={()=>handleGotoRoom(data?.id)} className='text-left mt-2 mb-2 cursor-pointer'>
            <h3 className='font-semibold text-xl my-2'>{data?.name}</h3>
                <p className=''>{data?.description}</p>
            </div>
            <div className='flex justify-between mt-6' >
                <div className='flex items-center'>
                    {
                    owner?<div></div>:joined?<div className={`flex justify-center items-center px-6 py-2 font-semibold rounded-full mx-2 ${drk_theme ? "bg-bg_dark_pri text-font_dark_pri":"bg-bg_light_pri text-font_light_pri"}`}>Joined</div>:<button onClick={()=>handleJoinRoom(data.id)} className='bg-third_color px-6 rounded-full text-semibold text-white text-center py-2 mr-4'>{join_loading?"Loading...":"Join"}</button>}
                    <GrGroup className='text-sm mx-2 text-gray-600' />
                    <p className='text-sm text-gray-600'> {data?.participants.length} Joined</p>
                </div>
                <div className='flex'>
                    <div className={`flex justify-center items-center px-6 py-2 font-semibold rounded-full mx-2 ${drk_theme ? "bg-bg_dark_pri text-font_dark_pri":"bg-bg_light_pri text-font_light_pri"}`} >{"title"}</div>
                </div>
            </div>
        </div>
    ) 
}

export default RoomCard